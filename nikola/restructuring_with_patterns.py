    ...
    if pattern is None:
        ...
        for document in data_dict:
            ...
            collection.update_one(
                {"app_id": app_id},
                {
                    "$addToSet": {"recommendations": new_recommendation},  # Dodaj u set recommendations
                    "$setOnInsert": {"app_id": app_id, "start_date": date},  # Postavi app_id i start_date ako se insertuje novi dokument
                    "$set": {"end_date": date}  # Postavi end_date na novu vrednost
                },
...
load_csv_to_mongodb('RecommendationsBucket', 'recommendations.csv', transform_recommendation, pattern="Bucket", chunk_size=50000, row_limit=1000000)
